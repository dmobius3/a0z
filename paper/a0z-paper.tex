...
% Author: B. Shatto (Independent Researcher)
...
%
% ---------------------------------------------------------------------
% TARGET-JOURNAL CLASS SWITCH (Foundations of Physics)
% ---------------------------------------------------------------------
% This file builds with article.cls for review/draft compilation.
% Foundations of Physics requires Springer's svjour3 class at
% submission time. svjour3.cls is not on CTAN; download Springer's
% submission package from
%
%   https://www.springer.com/journal/10701/submission-guidelines
%
% (look for "LaTeX Macro Package"). Drop svjour3.cls + svglov3.clo +
% spbasic.bst into this directory, then:
%
%   1. Comment out the article-class line below.
%   2. Uncomment the svjour3 line.
%   3. Adjust the title/author block (lines marked "FoP author block")
%      to use \institute and \email per svjour3 conventions.
%   4. Remove \usepackage{authblk} (svjour3 has its own author macros).
%   5. Recompile.
%
% See README.md "Submission to FoP" section for the exact swap.
% ---------------------------------------------------------------------

\documentclass[11pt,a4paper]{article}
% \documentclass[smallextended]{svjour3}    % <-- FoP submission
...
\author[1]{B.~Shatto%
\thanks{Email: \href{mailto:bshatto.pe@gmail.com}{bshatto.pe@gmail.com}.\
        ORCID: \href{https://orcid.org/0009-0007-4419-1311}{0009-0007-4419-1311}.}}
\affil[1]{Independent Researcher, St.~Petersburg, FL, USA}
...
tension is quantified in Section~\ref{sec:constraints} and held without
retreat.
...
(Figure~\ref{fig:five-exponents}). A measurement of any one observable
at any one redshift tests $\afid(z) = \afid(0)\,\Ez$ through that
channel; consistency across channels at the same $z$ tests the
universality of the deep-MOND scaling across observable classes.
...
\end{equation*}
Two features matter. First, the magnitudes overlap: both the prediction
and the data place the high-$z$ BTFR substantially below the local
relation. Second, the trend shapes disagree: the data are non-monotonic
($-0.44 \to -0.27$), while the prediction is strictly monotonic
($-0.227 \to -0.540$) (Figure~\ref{fig:ubler}).
...
non-monotonic. The 2.9$\sigma$ joint tension under the most
conservative systematics budget is held against the prediction without
retreat.}
\label{fig:ubler}
\end{figure}
...
quoted proxy, not a standalone relativistic lensing law. The
Section~\ref{sec:euclid} test is accordingly a test of the proxy-level
prediction, not of a relativistic lensing theory the framework does not
yet supply.
...
\small
\begin{tabular}{p{0.18\linewidth} p{0.22\linewidth} p{0.30\linewidth} p{0.20\linewidth}}
\toprule
...
\label{tab:schedule}
\small
\begin{tabular}{p{0.26\linewidth} p{0.22\linewidth} p{0.30\linewidth} p{0.14\linewidth}}
\toprule
...
shortens as $\Ez^{-1/4}$. The five exponents are correlated, not
independent: consistency across channels at the same redshift tests the
universality of the deep-MOND scaling.

The prediction lives in a constraint landscape with one genuine tension
({\"U}bler et al.\ KMOS3D BTFR trend shape at $2.9\sigma$, tested by
forward modeling and not dissolved), one inherited problem
...
is acknowledged and held against the prediction without retreat.
...
The selection rule is a postulate of the framework, not derived from
the topology. Its empirical support is the set of percent-level
agreements it produces: $\Lambda$ at $2\%$, $\alpha$ at $0.5\%$, and
the Milgrom ratio at $0.7\%$, across observables spanning 122 orders
of magnitude from a single rule.
